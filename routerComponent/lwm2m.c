...
static void swi_mangoh_data_router_avSvcFieldEventHandler(
    le_avdata_AssetInstanceRef_t, const char*, void*);

static void swi_mangoh_data_router_avSvcFieldEventHandler
(
    le_avdata_AssetInstanceRef_t assetInstanceRef,
...
)
{
    ...
    swi_mangoh_data_router_avSvcFieldEventUpdateHndlr_t* fieldEventHandler =
        le_hashmap_Get(avsvc->fieldEventHndlrs, fieldName);
    if (fieldEventHandler)
    {
        swi_mangoh_data_router_dbItem_t* dbItem =
            swi_mangoh_data_router_db_getDataItem(avsvc->db, fieldName);
        LE_ASSERT(dbItem);
...
void swi_mangoh_data_router_avSvcSessionStart
(
    const char* asset,
...
)
{
    ...
    avsvc->fieldEventHndlrs = le_hashmap_Create(
        SWI_MANGOH_DATA_ROUTER_LWM2M_FIELD_HANDLERS_MAP_NAME,
        SWI_MANGOH_DATA_ROUTER_LWM2M_FIELD_HANDLERS_MAP_SIZE,
...
)
{
    ...
    switch (dbItem->data.type)
    {
        ...
        case DATAROUTER_FLOAT:
            LE_DEBUG(
                "AVSVC <-- key('%s'), value(%f), timestamp(%lu)",
                key,
                dbItem->data.fValue,
                dbItem->data.timestamp);
    ...
    swi_mangoh_data_router_avSvcFieldEventUpdateHndlr_t* fieldEventHandler =
        le_hashmap_Get(avsvc->fieldEventHndlrs, key);
    if (!fieldEventHandler)
    {
        ...
        fieldEventHandler->handlerRef = le_avdata_AddFieldEventHandler(
            avsvc->assetInstanceRef, key, swi_mangoh_data_router_avSvcFieldEventHandler, avsvc);
...
void swi_mangoh_data_router_avSvcSessionEnd
(
    swi_mangoh_data_router_avsvc_t* avsvc
)
{
    ...
    if (!le_hashmap_isEmpty(avsvc->fieldEventHndlrs))
    {
        ...
        while (res == LE_OK)
        {
            swi_mangoh_data_router_avSvcFieldEventUpdateHndlr_t* fieldEventUpdateHndlr =
                (swi_mangoh_data_router_avSvcFieldEventUpdateHndlr_t*)le_hashmap_GetValue(iter);
